...
// Helper functions from api/index.ts (snake_case -> camelCase)
...
async function saveTenantToSupabase(updatedTenant: any): Promise<void> {
  ...
  if (updatedTenant.crmClients?.length) {
    console.log('Inserting crmClients...');
  ...
  if (updatedTenant.finance?.entries?.length) {
    console.log('Inserting finance entries...');
  ...
  if (updatedTenant.marketingCampaigns?.length) {
    console.log('Inserting marketingCampaigns...');
  ...
  if (updatedTenant.productsToSell?.length) {
    console.log('Inserting productsToSell...');
...
async function test() {
  const { data: tenantRow } = await supabase.from('tenants').select('*').eq('slug', 'sallesfit').single();
  const tenant = await fetchFullTenant(tenantRow);
  
  console.log('Executing saveTenantToSupabase...');
