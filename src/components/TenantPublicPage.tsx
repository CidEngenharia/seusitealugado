                ...
                <div className={isMinimal ? "divide-y divide-zinc-800/20" : "grid grid-cols-1 md:grid-cols-2 gap-4"}>
                  {tenant.services.map((service) => (
                    ...
                    >
                      ...
                      <div className="flex flex-col items-end pt-2 border-t border-zinc-800/60 gap-3">
                        <div className="flex flex-col items-end text-right space-y-0.5">
                          {service.priceOnRequest || service.price === 0 ? (
                            <span className="text-emerald-500 font-bold text-sm">Consultar</span>
                          ) : (
                            <span className="text-emerald-500 font-bold text-sm">R$ {service.price.toFixed(2)}</span>
                          )}
                          <span className={`text-[10px] ${themeMode === 'dark' ? 'text-zinc-400' : 'text-zinc-500'}`}>Duração: {service.duration} mins</span>
                        </div>
                        <div className="flex items-center gap-2 justify-end">
                          {/* Botão de WhatsApp direto com o número cadastrado no site */}
                          {(tenant.socials?.whatsapp || tenant.socials?.phone) && (
                            <a
                              href={`https://api.whatsapp.com/send?phone=55${(tenant.socials.whatsapp || tenant.socials.phone || "").replace(/\D/g, "")}&text=${encodeURIComponent(`Olá! Gostaria de mais informações sobre o serviço: ${service.name}`)}`}
                              target="_blank"
                              rel="noopener noreferrer"
                              className="p-2 rounded-lg bg-emerald-500 hover:bg-emerald-600 text-white transition-transform active:scale-95 shadow-sm flex items-center justify-center cursor-pointer"
                          ...
                          <button 
                            ...
                            className={`px-3 py-2 rounded-lg text-white font-semibold text-xs flex items-center gap-1 cursor-pointer transition-transform duration-100 active:scale-95 ${themeColors.bg} ${themeColors.bgHover}`}
                          >
                            <Calendar size={12} />
                            <span>Agendar</span>
                          </button>
